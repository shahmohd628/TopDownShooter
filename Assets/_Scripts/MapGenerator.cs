...
public class MapGenerator : MonoBehaviour {
	...
	public Transform obstaclePrefab;        // Single obstacle, no array
	...
	void Awake() {
		...
		if (navMeshSurface == null)
			Debug.LogError("No NavMeshSurface found in scene! Please add one to any GameObject.");
	...
	public void GenerateMap() {
		...
		for (int i = 0; i < obstacleCount; i++) {
			...
			if (randomCoord != currentMap.mapCentre && MapIsFullyAccessible(obstacleMap, currentObstacleCount)) {
				...
				Transform newObstacle = Instantiate(obstaclePrefab, obstaclePosition + Vector3.up, Quaternion.identity) as Transform;
	...
	public Transform GetRandomOpenTile() {
		if (shuffledOpenTileCoords == null || shuffledOpenTileCoords.Count == 0) {
			Debug.LogError("Open tiles not generated yet!");
			return tileMap[0,0]; 
		}
